...
export function MovementArrow({ movement, players }: MovementArrowProps) {
  const player = players.find((p) => p.id === movement.playerId);
  if (!player) return null;

  const { stroke, dash } = MOVEMENT_STYLES[movement.type];
  ...
  return (
    <line
      ...
      markerEnd={`url(#arrow-${movement.type})`}
      opacity={0.9}
    />
  );
}
...
export function PassArrow({ pass, players }: PassArrowProps) {
  const from = players.find((p) => p.id === pass.fromPlayerId);
  const to   = players.find((p) => p.id === pass.toPlayerId);
  if (!from || !to) return null;

  const { sx, sy, ex, ey } = shorten(from.x, from.y, to.x, to.y, 24, 24);
